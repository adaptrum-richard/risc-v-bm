 ...
 *
 * 编译kallsyms过程, 由如下步骤：
 * 1） 先link一个kernel_tmp.elf
 * 2） 读符号表，导出汇编文件：
 *     riscv64-linux-gnu-nm -n kernel_tmp.elf | scripts/kallsyms > tmp_kallsyms1.S
 *
 * 3)  编译汇编文件：
 *     riscv64-linux-gnu-gcc -c tmp_kallsyms1.S -o tmp_kallsyms1.o
 *
 ...
 *
 * 6） 编译汇编文件：
 *     riscv64-linux-gnu-gcc -c tmp_kallsyms2.S -o tmp_kallsyms2.o
 *
 * 7） 最后，加上tmp_kallsyms2.o来link成最终的kernel.elf。
 *
 * kallsyms表的查表过程。
 *
